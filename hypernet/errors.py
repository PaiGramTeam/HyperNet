...
class NetworkError(HyperNetException):
...
class BadRequest(HyperNetException):
    ...
    def response(self) -> dict[str, Union[str, Any, None]]:
        return {"code": self.ret_code, "message": self.original}
...
class InvalidTokens(CookieException):
    """Tokens weren't valid."""
...
class NotSupported(HyperNetException):
    """API not supported."""
...
def raise_for_ret_code(data: dict[str, Any]) -> NoReturn:
    ...
    """
    r, m = data.get("code", 0), data.get("message", "")
    ...
    if m.startswith("auth key") and r == -1:
        raise AuthkeyTimeout(data)
    if r == 10001 and "重复签到" in m:
        raise AlreadyClaimed(data)
